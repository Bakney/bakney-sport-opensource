<script>
    ...
    export let userData;
    export let visibleAssociationQuotes = false;

    export function toggleAssociationQuotes() {
        visibleAssociationQuotes = !visibleAssociationQuotes;
    }
</script>

{#if visibleAssociationQuotes}
    <div class="border rounded-lg" style="overflow-x:auto;">
        {#if userData.sport_association?.subscription_fee_plans?.length == 0}
        ...
        {/if}
        {#each userData.sport_association?.subscription_fee_plans || [] as subscription_fee_plan, idx}
                    ...
                    <div class="row">
                        <div class="col-md-12 col-lg-4">
                            ...
                            <div class="input-group input-group-solid">
                                <input
                                    bind:value={subscription_fee_plan.name}
                                    type="text"
                        ...
                        <div class="col-md-12 col-lg-4">
                            ...
                            <NumberInput
                                reactive={true}
                                disabled={subscription_fee_plan?.multiple_subscription_fee}
                                bind:value={subscription_fee_plan.subscription_fee}
                                name="subscription_fee_{subscription_fee_plan?.id}"
                                id="kt_inputmask_subscription_fee_{subscription_fee_plan?.id}"
                                placeholder="25,00"
                                checkFloat={true} />
                        </div>
                                    ...
                                    <span class="switch switch-icon ml-0">
                                        <label>
                                            <input
                                                type="checkbox"
                                                name=""
                                                bind:checked={subscription_fee_plan.advanced_options} />
                                            <span />
                                        </label>
                                    </span>
                                </div>
                        ...
                        <div class="col-md-12 col-lg-1 text-right d-flex justify-content-end align-items-center pt-4">
                            <DeleteButton
                                color="danger"
                                on:open={() => {
                                    userData.sport_association.subscription_fee_plans =
                                        userData.sport_association?.subscription_fee_plans?.filter(
                                            (item, index) => index !== idx
                                        );
                                }} />
                        </div>
                        <div class="col-12 mt-4" class:d-none={!subscription_fee_plan.advanced_options}>
                            <div class="align-items-center justify-content-start mt-4 d-flex">
                                <div class="mr-4">
                                    <span class="font-weight-bolder">Attiva dal</span>
                                    <DayMonthPicker
                                        bind:selectedDay={subscription_fee_plan.from_day}
                                        bind:selectedMonth={subscription_fee_plan.from_month} />
                                </div>
                                <div class="mr-4">
                                    <span class="font-weight-bolder">Fino al</span>
                                    <DayMonthPicker
                                        bind:selectedDay={subscription_fee_plan.to_day}
                                        bind:selectedMonth={subscription_fee_plan.to_month} />
                                </div>
                            </div>
                            <div class="col-12 px-0 mt-4 d-flex">
                                <div class="mr-4">
                                    ...
                                    <select
                                        bind:value={subscription_fee_plan.previous_subscription_fee_plan}
                                        class="form-control form-control-solid form-control-sm mt-2"
                                        style="width: 300px;">
                                        <option value="" default>Nessuna (è la prima)</option>
                                        {#each Array.from(userData.sport_association?.subscription_fee_plans)?.filter(x => x.id != subscription_fee_plan.id) as plan}
                                            <option value={plan?.id}>{plan?.name}</option>
                                        {/each}
                                    </select>
                                </div>
                                <div
                                    class="mr-4 {subscription_fee_plan.previous_subscription_fee_plan == ''
                                        ? 'd-none'
                                        : ''}">
                                    <span class=" font-weight-bolder">Emetti in automatico</span>
                                    <select
                                        bind:value={subscription_fee_plan.auto_assign}
                                        class="form-control form-control-solid form-control-sm mt-2">
                            ...
                            <div
                                ...
                                class:d-none={!subscription_fee_plan?.advanced_options}
                                class:d-flex={subscription_fee_plan?.advanced_options}>
                                <div class="mt-2">
                                    <div
                                        ...
                                        <div>
                                            La quota sarà visible dal <b>
                                                {subscription_fee_plan?.from_day}/{subscription_fee_plan?.from_month}</b>
                                            al
                                            <b>{subscription_fee_plan?.to_day}/{subscription_fee_plan?.to_month}</b>
                                            .
                                            {@html subscription_fee_plan?.previous_subscription_fee_plan != ''
                                                ? 'La quota antecedente è "' +
                                                  '<b>' +
                                                  userData.sport_association?.subscription_fee_plans?.find(
                                                      x => x.id == subscription_fee_plan?.previous_subscription_fee_plan
                                                  )?.name +
                                                  '"</b>. '
                                                : ''}
                                            {@html subscription_fee_plan?.auto_assign == '1' &&
                                            subscription_fee_plan?.previous_subscription_fee_plan != ''
                        ...
                        <div class="row mt-6">
                            <div class="col-12">
                                <SmartMultiSelect
                                    ...
                                    editable={false}
                                    active={false}
                                    bind:value={subscription_fee_plan.type_of_associate}
                                    props={{
                                        id: `subscription_fee_plan.type_of_associate.${idx}`,
                                        name: `subscription_fee_plan.type_of_associate.${idx}`,
                                        label: 'Valida per',
                                        ...
                                        value: subscription_fee_plan.type_of_associate,
                                    }} />
                            </div>
                            <div class="col-12">
                                <SmartMultiSelect
                                    ...
                                    editable={false}
                                    active={false}
                                    bind:value={subscription_fee_plan.type_of_request}
                                    props={{
                                        id: `subscription_fee_plan.type_of_request.${idx}`,
                                        name: `subscription_fee_plan.type_of_request.${idx}`,
                                        label: 'Per richieste',
                                        ...
                                        value: subscription_fee_plan.type_of_request,
                                    }} />
                            </div>
                        </div>
                    {/if}
                    {#if subscription_fee_plan?.installments}
                        <div
                            class="row mx-1 mt-1"
        ...
        <div class="row mx-1 mt-6 d-flex justify-content-center px-4 pb-4">
            <button
                ...
                on:click={() => {
                    let id = uuidv4();
                    if (!userData.sport_association.subscription_fee_plans)
                        userData.sport_association.subscription_fee_plans = [];
                    userData.sport_association.subscription_fee_plans = [
                        ...userData.sport_association.subscription_fee_plans,
                        {
    ...
    <div class="row mx-1 mt-6 d-flex justify-content-center px-4 pb-4">
        <button
            ...
            on:click={toggleAssociationQuotes}>
