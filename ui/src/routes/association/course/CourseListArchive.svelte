<script>
    ...
    window.disableCourse = id => {
        swal.fire({
            text: 'Vuoi spostare in bozza il corso?',
            icon: 'question',
            buttonsStyling: true,
            showCancelButton: true,
            cancelButtonText: 'Annulla',
            confirmButtonText: 'Sposta in bozza',
            reverseButtons: true,
        }).then(function (result) {
            if (result.isConfirmed) {
                KTApp.blockPage({
                    ...
                    template: function (row) {
                        ...
                        waitForElementAndExecute(`#action-col-${row.course_id}`, () => {
                            ...
                            if (!cannotDeleteCourse) {
                                let publishButton = new ArchiveButton({
                                    target: document.querySelector(`#action-col-${row.course_id}`),
                                    intro: true,
                                    props: {
                                        disabled: !canPerformAction('association.courses.update'),
                                        popover_text:
                                            "Rendi visibile il corso dal profilo dell'associazione ripristinandolo dall'archivio",
                                    },
                                });
                                ...
                                let moveToDraftButton = new EyeClosedButton({
                                    target: document.querySelector(`#action-col-${row.course_id}`),
                                    intro: true,
                                    props: {
                                        disabled: !canPerformAction('association.courses.update'),
                                        popover_text: "Nascondi il corso dal profilo dell'associazione",
                                    },
                                });
    ...
    onMount(() => {
        ...
        $courseListFilter.status_flag = 1;
    });
                ...
                <div class="card-title">
                    <h3 class="card-label font-size-h2">
                        ...
                        <span class="d-block text-muted pt-2 font-size-sm"
                            >Lista dei corsi e abbonamenti archiviati.</span>
                    </h3>
                </div>
